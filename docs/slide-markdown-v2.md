...
가로형 16:9 레이아웃, 여백 충분히.
텍스트는 한글 키워드 중심으로 짧게. 고유명사(Figma, Slack, AGENTS.md 등)와
약어(LLM, MCP, API)만 영어로. 한글 폰트는 손글씨 느낌이지만 또렷하게.
```
...
16:9 landscape layout, generous white space.
All text labels in KOREAN (Hangul), handwritten crayon style but clearly legible.
EXCEPTIONS — keep these in English only: proper nouns (Figma, Slack, Notion, Claude,
Cursor, Gmail, AGENTS.md, CLAUDE.md, SKILL.md) and abbreviations (LLM, MCP, API, JSX, USB-C).
```
...
Empty title space at the top and subtitle space at the bottom (for Korean text overlay later).
Crayon-drawn confetti and little stars scattered around.
Warm, inviting, storybook opening-scene feel. No text in the image itself.
```
...
### ③ 이미지 프롬프트
```
The cute AI robot sitting at a small wooden desk, sketching words onto a big piece
of paper with a crayon. Above the paper, a wobbly speech bubble shows the Korean
sentence "오늘 점심은 ___" with three floating word options below in Korean:
"김치찌개", "뭐 먹지", "어떻게". Each option has a hand-drawn percentage sticker
(62%, 21%, 8%). Crayon arrows point from the paper to the options.
A small label "LLM" sits in the corner. Warm ivory background,
pastel mint and coral accents, storybook infographic style.
All Korean text handwritten in a clear crayon style.
```
...
(1) a laptop showing a terminal window with crayon-drawn code lines, labeled "터미널" below,
(2) an IDE window with colorful code blocks and a sidebar, labeled "IDE" below,
(3) a desktop app window with a simple chat bubble interface, labeled "전용 앱" below.
Above all three, a big curved banner reads "Code Agent" (English kept as-is).
Under each frame, small product name tags in English: "Claude Code", "Cursor", "Claude Desktop".
At the bottom, a wobbly speech bubble in Korean: "전부 글로 입력!".
...
Pastel colors, cream background.
```
...
handwritten in Korean inside: a short one "이 함수 리팩토링해줘" and a longer one
"너는 시니어 백엔드 개발자야. 성능 이슈 3가지 찾아줘...". Each prompt has
a wobbly speech-bubble outline around it. The AI robot stands next to the window,
pointing at the text with a crayon and holding up a big sticker label in Korean
that reads "프롬프트". A small speech bubble near the robot in Korean says
"입력하는 모든 글자!". Soft cream background, pastel lavender and mint accents,
hand-drawn arrows linking robot to examples. All Korean text in clear crayon handwriting.
```
...
### ③ 이미지 프롬프트
```
Split comparison layout. Left side labeled in Korean "모호함" with a crayon X mark:
a fuzzy, messy output blob coming from a short Korean prompt "좋게 해줘".
Right side labeled in Korean "명확함" with a crayon check mark: a crisp, well-organized
output coming from a detailed Korean prompt "변수명은 camelCase, 에러는 try-catch로".
(Note: camelCase and try-catch kept in English as code terms.)
Below, four sticker-style badges in a row with Korean labels:
"명확한 지시", "역할 부여", "예시 제공", "단계별 사고".
The AI robot stands between the two sides, pointing to the "명확함" side with
a happy expression. Pastel hand-drawn style, cream background.
All Korean text handwritten in clear crayon style.
```
...
### ③ 이미지 프롬프트
```
Two big sketchy panels connected by multiple curved hand-drawn arrows going
back and forth. Left panel labeled in Korean "내 컴퓨터": a laptop with the AI
robot sitting next to it on a desk. Right panel labeled in Korean "클라우드 서버":
a fluffy crayon cloud with a bigger robot/LLM brain icon inside (labeled "LLM" in English).
Five numbered arrows (1-5) loop between them, each with a tiny handwritten Korean label:
"① 프롬프트", "② 파일 읽어줘?", "③ 파일 내용", "④ 이렇게 수정해", "⑤ 완료!".
Below the whole scene, a wobbly banner reads "Agentic Loop" (English, as it's a term).
Above the scene, a small Korean sticker note says "LLM은 내 컴퓨터를 직접 만지지 않아요".
Cream background, pastel colors, very hand-drawn feel. Korean text in clear crayon style.
```
...
(scratching its head), labeled in Korean "내 코드" at the bottom.
Right side: a floating Figma design mockup sketch — a simple UI with rectangles
and circles representing components, labeled "Figma" (English, brand name) at the bottom.
On the wall between them, three big hand-drawn question marks "???".
A thought bubble above the robot with a Korean label "어떻게 연결하지?".
Warm cream background, pastel coral and mint, storybook-style illustration.
Korean text in clear crayon handwriting.
```
...
reading "MCP" (English, abbreviation) and a smaller Korean subtitle
"AI를 위한 USB-C". Radiating outward from the port like a sunburst: cute
sticker-style icons of Figma, Slack, Notion, a database cylinder, and a Gmail
envelope — each connected by a hand-drawn cable (brand names in English).
At the bottom, three rounded boxes in a row with Korean labels:
"호스트 (Host)", "클라이언트 (Client)", "서버 (Server)", each with tiny example
icons inside (Cursor app icon, connector shape, Figma-style icon).
The AI robot stands on one side holding a flag sticker that reads "MCP".
Cream background, pastel colors, playful and educational feel.
Korean handwritten text, English used only for proper nouns and abbreviations.
```
...
Each stop has a tiny icon and a short Korean label:
(1) speech bubble "① 컴포넌트 만들어줘",
(2) robot thinking "② Agent 판단",
(3) connector plug "③ MCP 호출",
(4) Figma logo shape "④ Figma API",
(5) data flowing back "⑤ 데이터 받기",
(6) brain with "JSX" text "⑥ JSX 생성",
(7) file icon with checkmark "⑦ 파일 완성".
A hand-written curved banner above reads in Korean "Figma MCP 동작 흐름"
(Figma and MCP kept in English). The AI robot walks along the path at
the bottom, following the steps. Cream background, pastel mint and coral,
soft storybook infographic style. Korean text in clear crayon handwriting.
```
...
"😩" expression. Around its head, multiple overlapping wobbly speech bubbles
repeat the same rules in Korean (tool names stay in English):
"pnpm 써!", "npm 쓰지 마!", "src/legacy 건드리지 마!", "shadcn/ui 써!",
"any 타입 금지!". Some bubbles are crossed out showing they're being repeated
for the Nth time. A stack of crumpled papers on the desk suggests repetition.
A Korean handwritten label in the corner reads "또 똑같은 말...".
...
### ③ 이미지 프롬프트
```
Center: a big sketchy file icon labeled "AGENTS.md" (filename kept in English)
floating with a soft glow, showing a peek of the content inside written in Korean:
"## 하지 말 것", "- npm 금지", "- any 타입 금지".
The AI robot stands next to it with a relieved smiling expression, holding up
a check-mark sticker. A Korean speech bubble above the robot reads
"한 번만 써두면 끝!". At the bottom of the slide, two smaller file icons side by side:
"CLAUDE.md" with a Korean caption "Claude 전용" and "AGENTS.md" with a Korean
caption "오픈 표준", with a crayon arrow pointing from CLAUDE.md to AGENTS.md
showing the trend direction. Cream background, pastel mint accents, friendly style.
Korean text in clear handwriting, filenames and product names in English.
```
...
Left: "AGENTS.md" (filename in English) with a Korean caption
"항상 로드 — 요약 & 규칙".
Right: an open folder icon labeled "docs/" (folder name in English) with
smaller files inside ("architecture.md", "data-model.md"), and a Korean caption
"필요할 때만 — 상세 문서". A curved crayon arrow connects them.
Bottom half: a small checklist with sticker check marks (Korean items):
"✅ 명시적 Do/Don't", "✅ 짧고 명확한 문장", "✅ 선언적 문체".
And one crossed-out item in Korean: "❌ 가능하면... 해주세요".
A small Korean section title above the checklist: "요즘 md 트렌드".
The AI robot peeks from the corner holding a pencil.
Cream background, pastel palette. Korean text in clear handwriting.
```
...
Around the robot, three wobbly speech bubbles float with Korean complaints
(file formats stay in English):
"PDF 노하우는 어디나 똑같은데...",
"Excel 규칙 매번 복붙?",
"PPT 템플릿 또 반복?".
At the bottom edge, a new folder labeled "skills/" (folder name in English)
is peeking into the scene with a Korean sticker tag "새로운 해결책?",
slightly glowing. Cream background, pastel yellow highlighting the lightbulb,
soft sketch style. Korean text in clear crayon handwriting.
```
...
with "pdf-generation/" branching below, containing "SKILL.md" (with a tiny
pencil icon), "templates/" folder, "scripts/" folder. Drawn as a crayon tree
diagram. All folder/file names in English.
On the right, a callout bubble with a Korean title "점진적 공개 (Progressive Disclosure)"
explaining with two steps: a small "훑어보기" icon first, then a bigger "전체 로드" icon.
At the bottom, a row of other skill folder icons as stickers, each with Korean captions:
Excel spreadsheet ("엑셀 편집"), PPT slide ("PPT 생성"), bar chart ("차트 그리기").
The AI robot carries one of the folders like a precious package.
Cream background, pastel palette. Korean text in clear crayon handwriting,
code/file names in English.
```
...
### ③ 이미지 프롬프트
```
A hand-drawn comparison table with two columns. Column headers in English:
"AGENTS.md" (left) and "Skill" (right), written in chunky crayon letters.
Four rows with Korean labels on the far left:
- "범위" row: left cell "프로젝트 단위", right cell "작업 단위"
- "예시" row: left cell "pnpm 써", right cell "PDF 잘 만들기"
- "로드" row: left cell "항상", right cell "필요할 때만"
- "재사용" row: left cell "프로젝트 한정", right cell "여러 프로젝트 공유"
Each cell filled with tiny doodled icons and short Korean handwritten text.
The AI robot stands below the table holding two objects: AGENTS.md file in
left hand, a Skill folder in right hand, with a big Korean speech bubble
saying "둘 다 있어야 완전해!". Cream background, pastel dividers,
notebook/sketchbook aesthetic. Korean text in clear crayon handwriting.
```
...
Each stop has an icon and a Korean keyword label below (proper nouns kept in English):
① brain icon "LLM" / "확률 머신",
② laptop icon / "Code Agent",
③ speech bubble / "프롬프트",
④ circular arrow / "Agentic Loop",
⑤ USB-C port / "MCP",
⑥ file icon "AGENTS.md" / "규약",
⑦ folder package / "Skill" / "능력".
Above the roadmap, a big wobbly banner with a Korean question:
"글쓰는 확률 머신에게, 어떻게 더 좋은 코드를 쓰게 할까?".
The AI robot stands at the end of the path, waving goodbye with a Korean speech
bubble "감사합니다!". Crayon confetti and little hearts scattered around.
Warm cream background, full pastel palette, joyful closing-scene feel.
Korean handwritten text clear and legible.
```
...
3. **한글 텍스트 렌더링은 이미지 AI 종류마다 편차가 큽니다.**
...
4. **스크립트는 실제 말투**로 써놨으니, 본인 말투에 맞게 조금씩 다듬어 쓰시면 자연스럽습니다.
5. **슬라이드가 많다면**(15장) 장면 3·4(프롬프트)를 하나로 합치거나, 장면 6(AGENTS.md)의 3개를 2개로 줄여도 무방합니다.
